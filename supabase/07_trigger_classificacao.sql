-- ============================================================
-- Bolão Copa 2026 — Trigger: auto-update classificacao_grupos
-- Run AFTER 06_schema_v2.sql.
-- Fires on INSERT, UPDATE, DELETE of resultados.
-- Recalculates standings from scratch for the affected group.
-- ============================================================
...
DECLARE
  ...
  v_arr_len   INTEGER;
BEGIN
  -- Determine which jogo_id was affected
  ...
  LOOP
    ...
    v_arr := CASE WHEN TRIM(COALESCE(v_ult,'')) = '' THEN '{}'::TEXT[]
                  ELSE string_to_array(TRIM(v_ult), ' ')
             END;
    v_arr     := v_arr || ARRAY[v_res_a::TEXT];
    v_arr_len := array_length(v_arr, 1);
    IF v_arr_len > 3 THEN
      v_arr := v_arr[v_arr_len-2 : v_arr_len];
    END IF;
...
-- Attach trigger to resultados table
...
CREATE TRIGGER trg_update_classificacao_grupos
  ...
  FOR EACH ROW
  EXECUTE FUNCTION fn_update_classificacao_grupos();
