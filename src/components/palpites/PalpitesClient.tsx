  ...
  function ProgressBar({ earned, max, color = '#4A90D9' }: { earned: number; max: number; color?: string }) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 5 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', minWidth: 32, textAlign: 'right' }}>{pct}%</span>
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{ background: '#0D1E3D', border: '1px solid rgba(74,144,217,0.15)', borderRadius: 10, padding: '14px 16px', marginBottom: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 }}>
          ⚽ Pontos por fase
        </div>
          ...
          return (
            <div key={fase} style={{ paddingBottom: idx < arr.length - 1 ? 12 : 0, marginBottom: idx < arr.length - 1 ? 12 : 0, borderBottom: idx < arr.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                <div>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>{FASE_NAMES[fase]}</span>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginLeft: 8 }}>{sub}/{total} jogos</span>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: earned > 0 ? '#4A90D9' : 'rgba(255,255,255,0.25)' }}>
                    {earned}
                  </span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.50)', marginLeft: 3 }}>
                    / {max} pts
                  </span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.55)', minWidth: 32, textAlign: 'right' }}>{pct}%</span>
              </div>
            </div>
          )
        })}
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Subtotal jogos</span>
            <span style={{ fontSize: 13, fontWeight: 800, color: '#4A90D9' }}>{ptsJogos} / {maxJogosTotal} pts</span>
          </div>
      ...
      <div style={{ background: '#0D1E3D', border: '1px solid rgba(74,144,217,0.15)', borderRadius: 10, padding: '14px 16px', marginBottom: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 10 }}>
        ...
        {[
          ...
          <div key={item.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 0', borderBottom: idx < arr.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', fontWeight: 600 }}>{item.label}</div>
                <div style={{ fontSize: 12, color: item.value ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.2)', fontStyle: item.value ? 'normal' : 'italic' }}>
                  {item.value || '— não preenchido'}
                </div>
              </div>
            </div>
            <span style={{ fontSize: 11, color: 'rgba(255,200,80,0.6)', fontWeight: 700, whiteSpace: 'nowrap' }}>até {item.maxPts} pts</span>
          </div>
        ))}
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Subtotal especiais</span>
            <span style={{ fontSize: 13, fontWeight: 800, color: ptsEspeciais > 0 ? '#4A90D9' : 'rgba(255,255,255,0.25)' }}>
