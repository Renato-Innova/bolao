...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setMessage('Erro ao atualizar a senha. O link pode ter expirado.')
    } else {
    ...
    <div className="auth-page" style={{
      ...
      <div className="auth-card" style={{ width: '100%', maxWidth: 400, background: '#0D1E3D', border: '1px solid rgba(74,144,217,0.18)', borderRadius: 12, overflow: 'hidden', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(74,144,217,0.18)' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'white', textTransform: 'uppercase', letterSpacing: 0.5 }}>Criar nova senha</span>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {message && (
              <div style={{
                ...
                background: message.startsWith('Erro') || message.includes('coincidem') ? 'rgba(255,100,100,0.1)' : 'rgba(74,217,144,0.1)',
                border: `1px solid ${message.startsWith('Erro') || message.includes('coincidem') ? 'rgba(255,100,100,0.3)' : 'rgba(74,217,144,0.3)'}`,
                borderRadius: 8, fontSize: 13,
                color: message.startsWith('Erro') || message.includes('coincidem') ? 'rgba(255,130,130,0.9)' : 'rgba(130,255,180,0.9)',
              }}>
                {message}
              </div>
            )}
