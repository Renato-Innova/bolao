...
export default function NovaSenhaPage() {
  ...
  useEffect(() => {
    // O callback já trocou o code e colocou a sessão nos cookies.
    // Lemos a sessão aqui para confirmar que ela chegou ao browser.
    const supabase = createClient()
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) { setReady(true); return }

      // Fallback: aguarda evento de auth (caso o cookie ainda não tenha propagado)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Usa Server Action — lê a sessão de recovery diretamente dos cookies do servidor,
    // o que evita o erro 422 que ocorre com o client-side updateUser.
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="••••••••" autoComplete="new-password" className="auth-input" style={inp} />
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showConfirm ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)} required placeholder="••••••••" autoComplete="new-password" className="auth-input" style={inp} />
