...
export default function OperadorPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'todos' | 'ativo' | 'inativo'>('inativo')
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {(() => {
            ...
            const totalStatus = statusFilter === 'todos' ? palpites.length : palpites.filter(p => p.status === statusFilter).length
            return (
              <>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>
                  ...
                  {' · '}
                  <span style={{ color: '#4ade80' }}>{palpites.filter(p => p.status === 'ativo').length} ativos</span>
                  {' · '}
                  <span style={{ color: 'rgba(255,130,130,0.8)' }}>{palpites.filter(p => p.status === 'inativo').length} inativos</span>
                </div>
                {filtered.length === 0 && (
          ...
          {filtered.map(p => (
            <div key={p.id} style={{
              background: '#0D1E3D',
              border: `1px solid ${p.status === 'ativo' ? 'rgba(74,222,128,0.20)' : 'rgba(74,144,217,0.2)'}`,
              borderRadius: 10, padding: '12px 16px',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
            }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'white' }}>{p.nome}</div>
                  <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 7px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: 0.3, background: p.status === 'ativo' ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.07)', color: p.status === 'ativo' ? '#4ade80' : 'rgba(255,255,255,0.4)' }}>
